-- NIGHTOPS order history support for st_orders
...
create index if not exists st_orders_history_date_idx
  on public.st_orders (order_date, created_at desc);
create index if not exists st_orders_history_page_thread_idx
  on public.st_orders (page_id, thread_id, order_time desc);
create index if not exists st_orders_history_phone_idx
  on public.st_orders (phone);

comment on column public.st_orders.chat_timeline is
  'Order parser timeline snapshot; chat remains in chat_customer_messages/chat_page_messages';
comment on column public.st_orders.raw_text_with_phone is
  'Parser evidence snapshot used by order history, not a chat transcript source';
comment on column public.st_orders.raw_text_with_phone_timed is
  'Timestamped parser evidence snapshot for order audit';
comment on column public.st_orders.page_id is
  'Shared link key to chat page projection';
comment on column public.st_orders.thread_id is
