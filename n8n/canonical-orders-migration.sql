...
-- canonical_orders is the only trusted order source for the application.
-- Run this in Supabase SQL Editor before connecting the new n8n payload.
-- Do not delete canonical_orders or canonical_order_items yet; keep them as archived backup.
...
create index if not exists canonical_orders_canonical_created_idx
  on public.canonical_orders (created_at desc);
create index if not exists canonical_orders_canonical_order_number_idx
  on public.canonical_orders (order_number);
create index if not exists canonical_orders_canonical_page_thread_idx
...
update public.canonical_orders
...
comment on column public.canonical_orders.items_json is
  'Canonical ordered item lines; each object should include sku, th_name, label_display, quantity, unit_price';
comment on column public.canonical_orders.items_text is
  'Human-readable multi-item lines for packer and Telegram';
comment on column public.canonical_orders.total_quantity is
