...
-- The RPC intentionally returns central_order_master rows only. Product details remain in bb_order_items_fix.

create index if not exists central_order_master_page_thread_created_idx
  on public.central_order_master (page_id, thread_id, created_at desc);

create index if not exists central_order_master_page_id_idx
  on public.central_order_master (page_id);

create index if not exists central_order_master_thread_id_idx
  on public.central_order_master (thread_id);
...
returns setof public.central_order_master
language sql
stable
security invoker
...
as $$
  select o.*
  from public.central_order_master as o
