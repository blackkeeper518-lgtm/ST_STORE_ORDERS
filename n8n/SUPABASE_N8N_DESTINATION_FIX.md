...
Insert the Code node `🧹 PRODUCT_MAP_MASTER_SANITIZER` immediately before the `product_map_master` HTTP Request node. Configure the HTTP Request body as `={{ $json }}`. Do not add `address_display_packer` to the product table; keep addresses only in `canonical_orders`/`canonical_order_items`.
...
## 2. `canonical_orders` RLS error

The error `new row violates row-level security policy for table canonical_orders` is an authorization problem, not a missing-column problem. The n8n HTTP Request node is using a key that cannot insert into `canonical_orders`—usually an anon key or a credential attached to the wrong node.
...
Do not paste the service-role key into a Code node, workflow JSON, frontend, or chat. Replace the Header Auth credential used by `👑 canonical_orders2` and `👑 canonical_orders3`, which POST to `canonical_orders`, then execute the workflow again.
