...
-- n8n URL uses: /rest/v1/central_order_master?on_conflict=upsert_key
...
create unique index if not exists central_order_master_upsert_key_unique_idx
  on public.central_order_master (upsert_key);

comment on index public.central_order_master_upsert_key_unique_idx is
...
where schemaname = 'public'
  and tablename = 'central_order_master'
  and indexname = 'central_order_master_upsert_key_unique_idx';
...
-- Body: output from CENTRAL_ORDER_MASTER_CANONICAL_PAYLOAD
