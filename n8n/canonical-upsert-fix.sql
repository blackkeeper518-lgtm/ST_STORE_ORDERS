...
-- n8n URL uses: /rest/v1/canonical_orders?on_conflict=upsert_key
...
create unique index if not exists canonical_orders_upsert_key_unique_idx
  on public.canonical_orders (upsert_key);

comment on index public.canonical_orders_upsert_key_unique_idx is
...
where schemaname = 'public'
  and tablename = 'canonical_orders'
  and indexname = 'canonical_orders_upsert_key_unique_idx';

-- n8n HTTP Request settings:
-- Method: POST (or PATCH/upsert operation)
-- URL: /rest/v1/canonical_orders?on_conflict=upsert_key
-- Header: Prefer = resolution=merge-duplicates,return=minimal
-- Body: output from CANONICAL_SUPHABASS_PAYLOAD
