...
with base as (
  ...
  select
    c.*,
    case
      when c.source_time_text ~ '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2,4}[[:space:]]+[0-9]{1,2}:[0-9]{2}(:[0-9]{2})?$' then
        format('%s-%s-%s %s+07:00',
          case
            when length(split_part(split_part(c.source_time_text, ' ', 1), '/', 3)) = 2
              then (2500 + split_part(split_part(c.source_time_text, ' ', 1), '/', 3)::int - 543)::text
            else split_part(split_part(c.source_time_text, ' ', 1), '/', 3)
          end,
          lpad(split_part(split_part(c.source_time_text, ' ', 1), '/', 2), 2, '0'),
          lpad(split_part(split_part(c.source_time_text, ' ', 1), '/', 1), 2, '0'),
          case when length(split_part(c.source_time_text, ' ', 2)) = 5 then split_part(c.source_time_text, ' ', 2) || ':00' else split_part(c.source_time_text, ' ', 2) end
        )
      else c.source_time_text
    end as source_time_iso,
    (pm.master_sku is not null and coalesce(nullif(to_jsonb(pm)->>'master_display_for_packer', ''), nullif(to_jsonb(pm)->>'master_display', ''), nullif(to_jsonb(pm)->>'display_for_packer', '')) is not null) as master_matched, upper(regexp_replace(coalesce(nullif(to_jsonb(pm)->>'shipping_lane', ''), nullif(to_jsonb(pm)->>'delivery_lane', ''), nullif(to_jsonb(pm)->>'product_lane', ''), nullif(to_jsonb(pm)->>'temperature_zone', ''), nullif(to_jsonb(pm)->>'category', ''), nullif(to_jsonb(pm)->>'emoji', ''), ''), '[^A-Za-z0-9ก-๙]+', '', 'g')) as lane_key, coalesce(nullif(to_jsonb(pm)->>'master_display_for_packer', ''), nullif(to_jsonb(pm)->>'master_display', ''), nullif(to_jsonb(pm)->>'display_for_packer', '')) as master_display_for_packer, coalesce(nullif(to_jsonb(pm)->>'out_of_stock_joke', ''), nullif(to_jsonb(pm)->>'out_of_stock_notice', '')) as master_notice_override, case when jsonb_typeof(to_jsonb(pm)->'normal_header_variants') = 'array' and jsonb_array_length(to_jsonb(pm)->'normal_header_variants') > 0 then to_jsonb(pm)->'normal_header_variants'->>((abs(hashtext(coalesce(c.order_number_final, ''))) % jsonb_array_length(to_jsonb(pm)->'normal_header_variants'))::int) end as master_header_variant, pm.th_name as master_th_name, pm.stock_qty as master_stock_qty, pm.stock_status as master_stock_status,
    pm.stock_qty as stock_qty, pm.stock_status as stock_status, pm.stock_notice as stock_notice,
    ...
    case when pm.master_sku is not null and coalesce(nullif(to_jsonb(pm)->>'master_display_for_packer', ''), nullif(to_jsonb(pm)->>'master_display', ''), nullif(to_jsonb(pm)->>'display_for_packer', '')) is not null then 'MAPPED_FROM_PRODUCT_MASTER' else 'REVIEW_PRODUCT' end as mapping_status_fast,
    case
...
select
  ...
  d.product_text_final as n8n_product_display, coalesce(d.j->>'single_cleaned_products', d.j->>'single_cleaned_block') as single_cleaned_products, d.j->>'final_display_for_packer' as final_display_for_packer, d.master_display_for_packer as master_display_for_packer, d.product_text_final as product_evidence_display,
  case when d.product_stock_state = 'OUT_OF_STOCK' then coalesce(d.master_notice_override, d.master_stock_notice) when d.master_matched then d.master_display_for_packer else '🔮 [พักรายการ · ข้อมูลก้ำกึ่ง เหมือนความสัมพันธ์ที่อธิบายไม่ได้]' end as product_for_delivery, d.quantity_final as quantity_from_order_table, d.j->>'quantity' as quantity, d.j->>'qty' as qty, d.j->>'extracted_qty' as extracted_qty, d.j->>'master_qty_display' as master_qty_display,
  d.sku_final as sku, d.j->>'master_sku' as master_sku, d.lane_key, d.master_matched, d.master_th_name, d.master_stock_qty, d.master_stock_status, d.master_stock_qty as stock_qty, d.master_stock_status as stock_status, d.stock_notice as stock_notice, d.stock_notice as telegram_header, d.master_notice_override as master_stock_notice, d.product_stock_state,
  d.master_matched as is_mapped, d.mapping_status_fast, d.mapping_status_fast as mapping_status, d.mapping_status_fast as web_mapping_status, d.telegram_status_final as telegram_status,
  ...
  format(E'%s\n━━━━━━━━━━━━━━━━━━━━\n⏰ <b>เวลาสั่งซื้อ:</b> %s\n🆔 <b>เลขออเดอร์:</b> <code>%s</code>\n📢 <b>ชื่อเพจ:</b> %s\n👤 <b>Facebook:</b> %s\n💰 <b>ยอด COD:</b> <code>%s</code> บาท\n━━━━━━━━━━━━━━━━━━━━\n<code>%s</code>\n<code>%s</code>\n<code>%s</code>\n📦 <b>รายการสินค้า:</b>\n%s\n━━━━━━━━━━━━━━━━━━━━', coalesce(nullif(btrim(d.stock_notice), ''), nullif(btrim(d.master_header_variant), ''), '🚀 <b>[บิลสมบูรณ์ - ORDER_SNIPER_X]</b>'), coalesce(d.j->>'facebook_time_display', d.j->>'order_time_display', d.j->>'order_time', ''), d.order_number_final, coalesce(d.j->>'page_name',''), coalesce(d.j->>'facebook_name', d.j->>'customer_name',''), coalesce(d.j->>'cod_amount',''), coalesce(d.j->>'customer_name',''), coalesce(d.j->>'phone', d.j->>'extracted_phone',''), coalesce(d.address_final,'ไม่ระบุที่อยู่'), coalesce(nullif(btrim(d.j->>'final_display_for_packer'), ''), nullif(btrim(d.j->>'single_cleaned_products'), ''), nullif(btrim(d.j->>'single_cleaned_block'), ''), nullif(btrim(d.product_text_final), ''), 'ตรวจสอบสินค้า — ไม่ทิ้งรายการ')) as telegram_message_dynamic,
  d.j->>'telegram_message' as telegram_message, d.j->>'telegram_copy_text' as telegram_copy_text, d.j->'telegram_body' as telegram_body, d.j->>'telegram_sent' as telegram_sent
from dressed d;

create view public.vw_st_telegram_delivery_queue_fast as select *, 'PENDING_REVIEW_OR_SEND'::text as queue_status from public.vw_st_telegram_delivery_source_fast where not is_sent and source_time >= (((case when (now() at time zone 'Asia/Bangkok')::time >= time '22:00:00' then (now() at time zone 'Asia/Bangkok')::date else (now() at time zone 'Asia/Bangkok')::date - 1 end) + time '22:00:00') at time zone 'Asia/Bangkok');
create view public.vw_st_telegram_delivery_today_fast as select *, 'CURRENT_CUTOFF_SEND_READY'::text as queue_status from public.vw_st_telegram_delivery_source_fast where not is_sent and source_time >= (((case when (now() at time zone 'Asia/Bangkok')::time >= time '22:00:00' then (now() at time zone 'Asia/Bangkok')::date else (now() at time zone 'Asia/Bangkok')::date - 1 end) + time '22:00:00') at time zone 'Asia/Bangkok');
create view public.vw_st_telegram_delivery_yesterday_after_14_fast as select *, 'FROM_YESTERDAY_22_TO_TODAY_22_SEND_READY'::text as queue_status from public.vw_st_telegram_delivery_source_fast where not is_sent and source_time >= (((now() at time zone 'Asia/Bangkok')::date - 1) + time '22:00:00') at time zone 'Asia/Bangkok' and source_time < (((now() at time zone 'Asia/Bangkok')::date) + time '22:00:00') at time zone 'Asia/Bangkok';
...
comment on view public.vw_st_telegram_delivery_queue_fast is 'ST Telegram waiting queue; sent rows excluded; rolling cutoff starts after 22:00:00 Asia/Bangkok.';
...
comment on view public.vw_st_telegram_delivery_yesterday_after_14_fast is 'ST Telegram rolling room from yesterday 22:00 to today 22:00 Asia/Bangkok.';
