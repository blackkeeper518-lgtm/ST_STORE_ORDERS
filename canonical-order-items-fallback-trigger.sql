...
-- ใช้กับ public.canonical_orders หลังตรวจว่าคอลัมน์ telegram_body และ display_for_packer มีอยู่แล้ว
-- ไม่ลบ/ไม่เขียนทับ items_json ที่มีข้อมูลอยู่แล้ว

create or replace function public.canonical_orders_fill_items_json_fallback()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  -- fallback 2: legacy single product columns already present in canonical_orders
  if jsonb_array_length(candidate) = 0 and (
    ...
    candidate := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      ...
      'quantity', 1,
      'unit_price', null,
      'fallback_source', 'canonical_orders_legacy_columns'
    )));
  end if;
...
drop trigger if exists canonical_orders_fill_items_json_fallback on public.canonical_orders;
create trigger canonical_orders_fill_items_json_fallback
...
for each row execute function public.canonical_orders_fill_items_json_fallback();

-- ตรวจสอบรายการที่ยังว่างหลังติดตั้ง trigger
comment on function public.canonical_orders_fill_items_json_fallback() is
  'Fills empty canonical_orders.items_json from telegram_body payload or legacy product columns without overwriting non-empty items';
