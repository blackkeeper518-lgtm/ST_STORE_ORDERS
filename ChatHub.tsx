...
import { getOrderItems, itemDisplay } from "./order-items-fallback";
...
// --- Direct-to-Supabase reads (replace trpc.orders.threads / chat.messages) ---
// NOTE: filter columns confirmed from the real routers.ts you pasted earlier
// (chat_customer_messages, filtered by page_id + conversation_key, ordered
// by occurred_at). Everything else below (canonical_orders / chat_customer_evidence
// column names) is best-effort based on what the UI already expects — check
// the browser console if a query comes back empty and we'll adjust.
...
async function fetchThreads(): Promise<Thread[]> {
  ...
  for (const row of (data ?? []) as RawMessage[]) {
    if (!row.page_id || !row.conversation_key) continue;
    const key = `${row.page_id}:${row.conversation_key}`;
    ...
    byKey.set(key, {
      key,
      pageId: row.page_id,
      pageName: row.page_name || "ไม่ระบุเพจ",
      threadId: row.conversation_key,
      ...
      messageCount: 1,
      chatTimeline: [],
      searchText: `${row.customer_name ?? ""} ${row.text ?? ""}`,
      orders: [],
    });
  }
...
async function fetchMessages(pageId: string, threadId: string) {
  const { data, error } = await supabase
    .from("chat_customer_messages")
    .select("*")
    .eq("page_id", pageId)
    .eq("conversation_key", threadId)
    .order("occurred_at", { ascending: true });
  if (error) throw error;
  return (data ?? []).map((row: RawMessage) => ({
    id: row.id,
    text: row.text,
...
async function fetchLinkedOrders(pageId: string, customerName: string) {
  if (!customerName) return [];
  const { data, error } = await supabase.from("canonical_orders").select("*").eq("page_id", pageId).eq("customer_name", customerName);
  if (error) throw error;
  return (data ?? []).map(row => ({ ...row, items: getOrderItems(row) }));
}

async function fetchChatEvidence(pageId: string, threadId: string) {
  const { data, error } = await supabase.from("chat_customer_evidence").select("*").eq("page_id", pageId).eq("thread_id", threadId);
  if (error) throw error;
  return data ?? [];
}
...
export default function ChatHub() {
  ...
  const threadsQuery = useQuery({ queryKey: ["chat_threads"], queryFn: fetchThreads, refetchInterval: 60_000 });
  const threads = threadsQuery.data ?? [];
  const availablePages = useMemo<Array<[string, string]>>(
    () => Array.from(new Map(threads.filter(thread => thread.pageId && thread.pageName && thread.pageName !== "ไม่ระบุเพจ").map(thread => [thread.pageId, thread.pageName] as [string, string])).entries()).sort((a, b) => a[1].localeCompare(b[1], "th")),
    [threads],
  );
  const filteredThreads = useMemo(() => {
    const query = search.trim().toLowerCase();
    return threads.filter(thread => {
      const matchesPage = selectedPages.length === 0 || selectedPages.includes(thread.pageId);
      const matchesStatus = statusFilter === "all" || (statusFilter === "unread" ? thread.unread : thread.orderCount > 0);
      const matchesSearch = !query || [thread.customerName, thread.pageName, thread.preview, thread.searchText].some(value => String(value ?? "").toLowerCase().includes(query));
  ...
  const selectedPageId = selected?.pageId ?? "";
  const selectedThreadId = selected?.threadId ?? "";
  const selectedCustomerId = selected?.customerId ?? "";

  const messagesQuery = useQuery({ queryKey: ["chat_messages", selectedPageId, selectedThreadId], queryFn: () => fetchMessages(selectedPageId, selectedThreadId), enabled: Boolean(selectedPageId && selectedThreadId), refetchInterval: 10_000 });
  const linkedOrdersQuery = useQuery({ queryKey: ["linked_orders", selectedPageId, selected?.customerName], queryFn: () => fetchLinkedOrders(selectedPageId, selected?.customerName ?? ""), enabled: Boolean(selectedPageId && selected?.customerName && orderDetailOpen) });
  const chatEvidenceQuery = useQuery({ queryKey: ["chat_evidence", selectedPageId, selectedThreadId], queryFn: () => fetchChatEvidence(selectedPageId, selectedThreadId), enabled: Boolean(selectedPageId && selectedThreadId && orderDetailOpen) });
  const totalMessages = useMemo(() => threads.reduce((total, thread) => total + thread.messageCount, 0), [threads]);

  // --- Actions that need the server are disabled — no webhook/server in this setup ---
  const composerDisabledReason = "ปิดใช้งาน: การตอบแชทลูกค้าต้องมี server ส่งผ่าน Meta API ซึ่งไม่ได้เปิดใช้ในระบบนี้";
  const sendCurrentReply = () => window.alert(composerDisabledReason);
  const confirmSelectedOrder = () => window.alert("ปิดใช้งาน: การยืนยันออเดอร์จากแชทต้องมี server");
  ...
  const composer = <div className="border-t border-violet-500/10 pt-2">
    ...
    <div className="flex gap-2"><Input value={replyText} onChange={event => setReplyText(event.target.value)} placeholder="ปิดใช้งาน: ไม่มี server สำหรับส่งข้อความ" disabled className="border-violet-500/15 bg-black/25 text-white placeholder:text-violet-100/25" /><Button aria-label="ส่งข้อความ" disabled className="shrink-0 rounded-xl bg-gradient-to-r from-fuchsia-600 to-violet-600 opacity-50"><Send className="h-4 w-4" /></Button></div>
    <div className="mt-1 flex flex-wrap items-center gap-2"><input ref={fileInputRef} type="file" className="hidden" disabled /><Button type="button" size="sm" variant="outline" disabled className="h-7 border-violet-500/15 bg-black/20 text-[10px] text-violet-100/40"><ImagePlus className="mr-1 h-3.5 w-3.5" />แนบรูป (ปิดใช้งาน)</Button></div>
    <p className="mt-2 text-[10px] text-amber-300/70">หน้านี้เป็นโหมดอ่านอย่างเดียว (เชื่อมตรงจาก browser ไม่มี server) — ตอบลูกค้าผ่าน Facebook/Meta Business Suite โดยตรงแทน</p>
  </div>;

  const conversation = selected ? <div className="flex min-h-0 flex-col overflow-hidden">
    <div className="flex items-center justify-between gap-3 border-b border-violet-500/10 px-5 py-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500/25 to-violet-500/25 text-fuchsia-200"><CircleUserRound className="h-5 w-5" /></div><div className="min-w-0"><h2 className="truncate text-base font-semibold text-white">{selected.customerName || "ลูกค้าใหม่"}</h2><div className="mt-1 flex items-center gap-2 text-[11px] text-violet-100/40"><span className="truncate">{selected.pageName}</span><span>·</span><span>{timeLabel(selected.latestAt)}</span></div></div></div><div className="flex items-center gap-2"><Badge className="hidden border border-emerald-400/25 bg-emerald-400/10 text-emerald-300 sm:inline-flex">● ONLINE</Badge><Button size="sm" variant="outline" onClick={confirmSelectedOrder} className="rounded-xl border-cyan-400/25 bg-cyan-400/10 text-xs text-cyan-200 hover:bg-cyan-400/20 opacity-60">ยืนยันเป็นออเดอร์</Button><Button size="sm" variant="outline" onClick={() => setOrderDetailOpen(true)} className="rounded-xl border-amber-400/25 bg-amber-400/10 text-xs text-amber-200 hover:bg-amber-400/20">ออเดอร์ ({linkedOrdersQuery.data?.length ?? 0})</Button>{isMobile ? <button onClick={() => setMobileOpen(false)} className="rounded-xl p-2 text-violet-100/55 hover:bg-violet-500/10 hover:text-white" aria-label="ปิดห้องแชท"><X className="h-5 w-5" /></button> : null}</div></div>
  ...
  return <div className="min-h-[calc(100vh-2rem)] bg-[#09070d] text-white"><div className="mx-auto max-w-[1600px] space-y-5 p-2 sm:p-4 lg:p-6">
    ...
    <footer className="flex items-center gap-2 px-2 text-[11px] text-violet-100/30"><MessageCircle className="h-3.5 w-3.5" />เชื่อมตรงจาก browser · โหมดอ่านอย่างเดียว (ไม่มี server ส่งข้อความ)</footer>
  </div></div>;
}
