...
import { Activity, Archive, Bell, BrainCircuit, Boxes, CalendarDays, Database, LayoutDashboard, Link2, LockKeyhole, MessageCircle, PanelLeft, Send, Tags, Truck } from "lucide-react";
import { CSSProperties, useEffect, useRef, useState } from "react";
import { useLocation } from "wouter";
import { DashboardLayoutSkeleton } from "./DashboardLayoutSkeleton";
import { Button } from "./ui/button";
import { PRESENTATION_MODE } from "@shared/presentation";
import { getActiveCamp, getSupabaseConfig, readCanonicalOrders, type Camp } from "@/lib/canonical";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider className="h-full min-h-0 w-full min-w-0 overflow-hidden bg-[#09070d] text-white" style={{ "--sidebar-width": `${sidebarWidth}px` } as CSSProperties}><DashboardLayoutContent setSidebarWidth={setSidebarWidth}>{children}</DashboardLayoutContent></SidebarProvider>;
}
...
function DashboardLayoutContent({ children, setSidebarWidth }: DashboardLayoutContentProps) {
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      try {
        ...
        const rows = (result.orders ?? []).slice(0, 12) as Array<Record<string, any>>;
        const storageKey = `drakside-last-order-${getActiveCamp()}`;
        ...
        if (!cancelled) {
          ...
          setUnreadThreads(next.filter((item) => !item.seen).map((item) => ({ key: item.key, latestAt: item.time, customerName: item.title })));
          if (!lastSeen && next[0]?.key) localStorage.setItem(storageKey, next[0].key);
        }
      } catch { /* notification center must not block the app */ }
    };
  ...
  const st = camp === "ST";
  return <div className="singto-cyber-edge"><><div className="relative" ref={sidebarRef}><Sidebar collapsible="icon" className={st ? "border-r border-orange-500/20 bg-[#100b08] text-orange-50 [&_[data-slot=sidebar-inner]]:bg-[#100b08] [&_[data-slot=sidebar-inner]]:text-orange-50" : "border-r border-orange-500/10 bg-[#0d0a12] text-orange-50 [&_[data-slot=sidebar-inner]]:bg-[#0d0a12] [&_[data-slot=sidebar-inner]]:text-orange-50"} disableTransition={isResizing}><SidebarHeader className="h-16 justify-center"><div className="flex w-full items-center gap-3 px-2"><button onClick={toggleSidebar} className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${st ? "text-orange-200/70 hover:bg-orange-500/10 hover:text-orange-100" : "text-orange-200/60 hover:bg-orange-500/10 hover:text-orange-200"}`} aria-label="เปิดหรือปิดเมนู"><PanelLeft className="h-4 w-4" /></button>{!isCollapsed && <div className="flex min-w-0 items-center gap-2"><button type="button" onClick={() => setLocation("/secret-gallery")} className="group flex min-w-0 items-center gap-2" title="ห้องลับคลังโปรเจค"><span className={`truncate font-semibold tracking-tight ${st ? "text-orange-50 group-hover:text-orange-200" : "text-orange-50 group-hover:text-amber-200"}`}>{st ? "ST SINGTO STORE" : "BB STORE"}</span><LockKeyhole className={`h-3.5 w-3.5 ${st ? "text-orange-300/70 group-hover:text-orange-200" : "text-orange-300/60 group-hover:text-amber-300"}`} /></button></div>}</div></SidebarHeader><SidebarContent className="gap-0"><SidebarMenu className="px-2 py-1">{menuItems.map(item => <SidebarMenuItem key={item.path}><SidebarMenuButton isActive={location === item.path} onClick={() => setLocation(item.path)} tooltip={item.label} className={`h-10 font-normal ${st ? "text-orange-100/60 hover:bg-orange-500/10 hover:text-orange-50 data-[active=true]:bg-orange-500/15 data-[active=true]:text-orange-200" : "text-orange-100/55 hover:bg-orange-500/10 hover:text-orange-50 data-[active=true]:bg-orange-500/10 data-[active=true]:text-orange-200"}`}><item.icon className={`h-4 w-4 ${location === item.path ? (st ? "text-orange-300" : "text-orange-300") : ""}`} /><span>{item.label}</span>{item.path === "/chats" && unreadThreads.length > 0 ? <span className="ml-auto rounded-full bg-red-500/20 px-1.5 text-[10px] font-semibold text-red-300">{unreadThreads.length}</span> : null}</SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu><div className="mx-3 my-2 rounded-xl border border-red-400/15 bg-red-500/[0.04] p-2.5"><button type="button" onClick={() => { setNotificationsOpen((value) => !value); if (typeof Notification !== "undefined" && Notification.permission === "default") void Notification.requestPermission(); }} className="flex w-full items-center gap-2 text-left text-xs text-red-200"><Bell className="h-3.5 w-3.5" /><span>{unreadThreads.length ? `มีรายการใหม่ ${unreadThreads.length}` : "การแจ้งเตือน"}</span></button></div></SidebarContent><SidebarFooter className="p-3"><div className="flex items-center gap-3 rounded-lg px-1 py-1"><Avatar className={`h-9 w-9 shrink-0 ${st ? "border border-orange-400/30 bg-orange-500/10" : "border border-orange-400/20 bg-orange-500/10"}`}><AvatarFallback className={`bg-transparent text-xs font-medium ${st ? "text-orange-200" : "text-amber-200"}`}>{st ? "ST" : "BB"}</AvatarFallback></Avatar><div className="min-w-0"><p className={`truncate text-sm font-medium leading-none ${st ? "text-orange-100" : "text-amber-100"}`}>{st ? "ST SINGTO STORE" : "BB STORE"} · {camp}</p><p className={`mt-1.5 truncate text-xs ${st ? "text-orange-100/35" : "text-orange-100/35"}`}>{getSupabaseConfig(camp) ? "connected" : "configure /connect"}</p></div></div></SidebarFooter></Sidebar><div className={`absolute right-0 top-0 h-full w-1 cursor-col-resize hover:bg-primary/20 ${isCollapsed ? "hidden" : ""}`} style={{ zIndex: 50 }} onMouseDown={() => !isCollapsed && setIsResizing(true)} /></div><SidebarInset className={`min-h-0 min-w-0 overflow-hidden ${st ? "bg-[#090706]" : "bg-[#09070d]"}`}>{isMobile && <div className={`sticky top-0 z-40 flex h-14 items-center justify-between px-2 backdrop-blur ${st ? "border-b border-orange-500/20 bg-[#100b08]/95" : "border-b border-orange-500/10 bg-[#0d0a12]/95"}`}><div className="flex items-center gap-2"><SidebarTrigger className={`h-9 w-9 rounded-lg ${st ? "text-orange-100 hover:bg-orange-500/10" : "text-orange-100 hover:bg-orange-500/10"}`} /><span className={st ? "text-orange-50" : "text-orange-50"}>{activeMenuItem?.label ?? "เมนู"}</span></div></div>}<div className={`mx-4 mt-3 flex flex-wrap items-center gap-2 rounded-2xl px-3 py-2 text-[11px] ${st ? "border border-orange-500/20 bg-[#160e08] text-orange-100/75" : "border border-orange-500/20 bg-[#100c19] text-orange-100/75"}`}><span className={`font-medium ${st ? "text-orange-200" : "text-amber-200"}`}>{thaiDateTime}</span><span className={st ? "text-orange-100/35" : "text-orange-100/35"}>·</span><button type="button" onClick={() => { setNotificationsOpen((value) => !value); if (typeof Notification !== "undefined" && Notification.permission === "default") void Notification.requestPermission(); }} className="inline-flex items-center gap-1.5 text-red-100 hover:text-white"><Bell className="h-3.5 w-3.5" />{unreadThreads.length ? `มีรายการใหม่ ${unreadThreads.length}` : "ไม่มีรายการใหม่"}</button><span className={st ? "text-orange-100/30" : "text-orange-100/30"}>·</span><span className={st ? "text-orange-300" : "text-amber-300"}>ฐานข้อมูล Deployment นี้</span><span className={st ? "text-orange-100/45" : "text-orange-100/45"}>ฐานข้อมูลของ Deployment นี้</span><span className={st ? "text-orange-100/45" : "text-orange-100/45"}>Live 60s</span></div>{notificationsOpen && <div className="absolute right-4 top-16 z-50 w-[min(380px,calc(100vw-2rem))] rounded-2xl border border-orange-400/25 bg-[#120b16]/95 p-3 shadow-2xl backdrop-blur-xl"><div className="mb-2 flex items-center justify-between"><b className="text-sm text-orange-100">ศูนย์แจ้งเตือน · {st ? "ST SINGTO STORE" : "BB STORE"}</b><button type="button" onClick={() => { setNotificationsOpen(false); if (notifications[0]?.key) localStorage.setItem(`drakside-last-order-${getActiveCamp()}`, notifications[0].key); setUnreadThreads([]); setNotifications((items) => items.map((item) => ({ ...item, seen: true }))); }} className="text-[10px] text-orange-200/60 hover:text-orange-100">อ่านแล้ว</button></div>{notifications.length ? <div className="max-h-80 space-y-2 overflow-auto">{notifications.map((item) => <button type="button" key={item.key} onClick={() => { setLocation("/orders"); setNotificationsOpen(false); }} className={`w-full rounded-xl border p-3 text-left ${item.seen ? "border-white/10 bg-black/20" : "animate-pulse border-orange-300/50 bg-orange-400/10"}`}><p className="text-xs font-semibold text-orange-100">{item.title}</p><p className="mt-1 line-clamp-2 text-[11px] text-orange-100/60">{item.detail}</p></button>)}</div> : <p className="p-4 text-center text-xs text-orange-100/50">ยังไม่มีรายการใหม่</p>}</div>}<main className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-4">{children}</main></SidebarInset></></div>;
}
