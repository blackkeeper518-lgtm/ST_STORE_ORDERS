...
import { Activity, Archive, Bell, Boxes, CalendarDays, Database, KeyRound, LayoutDashboard, LogOut, MessageCircle, PanelLeft, Tags, Truck, Users } from "lucide-react";
...
const menuItems = [
  ...
  { icon: Users, label: "ประวัติลูกค้า", path: "/customer-history" },
  { icon: Truck, label: "ตรวจเลขพัสดุ", path: "/tracking-review" },
  ...
  { icon: KeyRound, label: "เชื่อมต่อ Supabase", path: "/connect" },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider data-store={storeConfig.id} className={`bg-[#09070d] text-white store-${storeConfig.theme}`} style={{ "--sidebar-width": `${sidebarWidth}px` } as CSSProperties}><DashboardLayoutContent setSidebarWidth={setSidebarWidth}>{children}</DashboardLayoutContent></SidebarProvider>;
}
...
function DashboardLayoutContent({ children, setSidebarWidth }: DashboardLayoutContentProps) {
  ...
  const isConnectPage = location === "/connect";
  const threadsQuery = trpc.orders.threads.useQuery(undefined, { refetchInterval: 15_000, enabled: !isConnectPage });
  const deliveryQuery = trpc.chat.deliveryHealth.useQuery(undefined, { refetchInterval: 15_000, enabled: !isConnectPage && user?.role === "admin" });
  ...
  useEffect(() => {
    ...
    if (typeof Notification !== "undefined" && Notification.permission === "granted") {
      ...
      new Notification(`${storeConfig.name} · ลูกค้าทักใหม่`, { body: `${newest?.customerName || "ลูกค้า"} ส่งข้อความใหม่เข้ามา` });
    }
  }, [unreadThreads]);
  ...
  return <>
    <div className="relative" ref={sidebarRef}>
      <Sidebar collapsible="icon" className="border-r border-violet-500/10 bg-[#0d0a12] text-violet-50 [&_[data-slot=sidebar-inner]]:bg-[#0d0a12] [&_[data-slot=sidebar-inner]]:text-violet-50" disableTransition={isResizing}>
        <SidebarHeader className="h-16 justify-center"><div className="flex w-full items-center gap-3 px-2"><button onClick={toggleSidebar} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-violet-200/60 hover:bg-violet-500/10 hover:text-fuchsia-200" aria-label="เปิดหรือปิดเมนู"><PanelLeft className="h-4 w-4" /></button>{!isCollapsed && <div className="flex min-w-0 items-center gap-2"><span className="min-w-0"><span className="block truncate font-semibold tracking-tight text-violet-50">{storeConfig.name}</span><span className="block truncate text-[9px] font-normal leading-3 text-violet-100/45">{storeConfig.tagline}</span></span></div>}</div></SidebarHeader>
