          ...
          <CardContent><div className="max-h-[640px] space-y-2 overflow-auto pr-1">
            ...
            {waiting.map((item, index) => {
              ...
              return <button type="button" key={item.upsert_key || item.order_number || item.id || index} onClick={() => setSelected(index)} className={`w-full rounded-2xl border p-3 text-left ${index === selected ? "border-orange-300/70 bg-orange-500/15 shadow-lg shadow-orange-950/20" : "border-white/10 bg-black/20 hover:border-orange-400/30"}`}>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-1.5"><span className={`rounded-full border px-2 py-0.5 text-[10px] ${deliveryStatus(item).tone}`}>{deliveryStatus(item).label}</span>{issues.length > 0 && <span className="rounded-full bg-red-400/10 px-2 py-0.5 text-[10px] text-red-200">ต้องตรวจ {issues.length}</span>}</div><p className="mt-1 text-[10px] text-orange-100/40">{deliveryStatus(item).slogan}</p>
              </button>;
            })}
          </div></CardContent>
        </Card>
