...
export default function ChatHub() {
  ...
  const composer = <div className="border-t border-orange-500/10 pt-2">
    ...
    <div className="flex items-end gap-2"><Textarea value={replyText} onChange={event => setReplyText(event.target.value)} placeholder={selectedCustomerId ? "พิมพ์ตอบลูกค้า…" : "ไม่พบ Customer PSID ของห้องนี้"} disabled={!selectedCustomerId} className="min-h-24 resize-y border-orange-500/15 bg-[#171c21] text-sm leading-6 text-white placeholder:text-orange-100/25 sm:min-h-28 lg:min-h-32" onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendCurrentReply(); } }} /><Button aria-label="ส่งข้อความ" disabled={sendReply.isPending || !selectedCustomerId || (!replyText.trim() && !replyImageUrl.trim() && !stickerId.trim())} onClick={sendCurrentReply} className="h-11 shrink-0 rounded-xl bg-amber-500 text-[#1c1302]"><Send className="h-4 w-4" /></Button></div>
    <div className="mt-1 flex flex-wrap items-center gap-2"><input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden" onChange={event => { handleImageUpload(event.target.files?.[0]); event.currentTarget.value = ""; }} /><Button type="button" size="sm" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploadImage.isPending} className="h-7 border-orange-500/15 bg-black/20 text-[10px] text-orange-100/70"><ImagePlus className="mr-1 h-3.5 w-3.5" />{uploadImage.isPending ? "กำลังอัปโหลด…" : "แนบรูป"}</Button>{replyImageUrl ? <span className="max-w-40 truncate text-[10px] text-emerald-300">รูปพร้อมส่ง</span> : null}<Input value={stickerId} onChange={event => setStickerId(event.target.value)} placeholder="Sticker ID (ถ้ามี)" className="h-7 w-44 border-orange-500/10 bg-black/20 text-xs text-white placeholder:text-orange-100/20" /><span className="whitespace-nowrap text-[10px] text-orange-100/25">Meta</span><Button type="button" size="sm" variant="outline" onClick={() => simulate("text")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-orange-400/20 bg-orange-400/[0.04] text-[10px] text-amber-200">จำลองข้อความ</Button><Button type="button" size="sm" variant="outline" onClick={() => simulate("image")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-orange-400/20 bg-orange-400/[0.04] text-[10px] text-amber-200">จำลองรูป</Button></div>
    {simulationPreview ? <div className="mt-2 rounded-xl border border-orange-400/20 bg-orange-400/[0.04] p-2"><div className="flex items-center justify-between text-[10px] text-amber-200"><span>DRY RUN · ไม่ได้ส่งจริง</span><button type="button" onClick={() => setSimulationPreview(null)} className="text-amber-100/50">ปิด</button></div><pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap text-[10px] text-amber-100/70">{simulationPreview}</pre></div> : null}
