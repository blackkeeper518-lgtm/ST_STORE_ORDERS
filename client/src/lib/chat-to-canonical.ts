...
export async function createCanonicalFromChat(input: { pageId: string; threadId: string; pageName?: string; customerName?: string }): Promise<ChatCanonicalResult> {
  ...
  const { data: saved, error: saveError } = await api.from("central_order_master").upsert(header, { onConflict: "upsert_key" }).select("id").single(); if (saveError) throw saveError; const orderId = saved.id;
