...
export async function readAlienReview(search = ''): Promise<AlienReviewItem[]> {
  ...
  return (itemsResult.data ?? []).map((item: any) => {
    const master = (item.product_id != null ? byId.get(String(item.product_id)) : undefined) ?? bySku.get(String(item.sku ?? '').trim().toLowerCase());
    ...
    const sku = String(item.sku ?? master?.sku ?? '').trim();
    const rawIsSku = Boolean(raw && sku && raw.toLowerCase() === sku.toLowerCase());
    const audit_status = !raw ? 'RAW_MISSING' : rawIsSku ? 'RAW_EQUALS_SKU' : (item.mapping_status ?? item.match_status ?? 'REVIEW');
    return { ...item, product_master: master ?? null, audit_status, raw_display: raw || 'ไม่มีคำดิบ', mapped_display: mapped || 'ยังไม่มีชื่อมาตรฐาน' };
