...
function normalizeOrder(row: Record<string, unknown>, items: LiveOrderItem[]): LiveOrder {
  ...
  return {
    ...
    chat_timeline: timeline(row.chat_timeline ?? row.web_chat_timeline ?? bodyField(row, "chat_timeline") ?? (row.source_payload && typeof row.source_payload === "object" ? (row.source_payload as Record<string, unknown>).chat_timeline : undefined) ?? row.raw_text_with_phone_timed ?? row.full_chunk_text ?? row.clean_text ?? row.telegram_copy_text ?? row.telegram_message),
