...
import {
  ...
  listProductAliasesFromSupabase,
  createProductAliasInSupabase,
  updateProductAliasInSupabase,
  updateStockProduct,
...
export const appRouter = router({
  ...
  orders: router({
    ...
    updateCustomer: publicProcedure.input(z.object({ id: z.number().int(), customerName: z.string().optional(), phone: z.string().optional(), fullAddress: z.string().optional(), codAmount: z.number().optional() })).mutation(({ input }) => updateLiveOrderCustomer(input.id, input)),
  ...
  productAliases: router({
    list: publicProcedure.query(() => listProductAliasesFromSupabase()),
    catalog: publicProcedure.query(() => fetchLiveProductMappings()),
    create: publicProcedure.input(z.object({ alias: z.string().min(1), canonicalSku: z.string().min(1), canonicalLabel: z.string().min(1) })).mutation(({ input }) => createProductAliasInSupabase(input)),
    update: publicProcedure.input(z.object({ id: z.number().int(), alias: z.string().min(1), canonicalSku: z.string().min(1), canonicalLabel: z.string().min(1), isActive: z.boolean().optional() })).mutation(({ input }) => updateProductAliasInSupabase(input.id, input)),
  }),
