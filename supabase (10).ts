...
export async function fetchLiveProductMappings(): Promise<LiveProductMapping[]> {
  const [productResult, mapResult, orderResult] = await Promise.allSettled([
    ...
    getRows<Record<string, unknown>>(CENTRAL_ORDER_TABLE, "*", 3000),
  ]);
  ...
  if (orderResult.status === "fulfilled") for (const order of orderResult.value) {
    for (const item of itemLinesFromOrder(order)) {
      const sku = String(item.sku ?? "").trim();
      const candidate = String(item.th_name ?? item.product_name ?? item.label_display ?? "").trim();
      if (sku && candidate) {
        const values = observed.get(sku) ?? new Set<string>();
        values.add(candidate);
        observed.set(sku, values);
      }
    }
  }
  return productResult.value.map(row => ({
    sku: String(row.sku ?? ""),
    ...
    observedAliases: Array.from(observed.get(String(row.sku ?? "").trim()) ?? []),
...
export async function updateProductMapAlias(sku: string, alias: string) {
  ...
  // Mapping rows are one-alias-per-row. Do not PATCH by SKU here, or one
  // alias edit would overwrite every alias belonging to that SKU.
...
export async function listProductAliasesFromSupabase() {
  // Join the two real tables in memory so an old PostgREST view cannot break
  // every mapping screen during a schema migration.
  const [rows, products] = await Promise.all([
    getRows<Record<string, unknown>>("product_map_master", "map_id,alias,alias_text,alias_norm,sku,mapping_status,match_confidence,source,source_room,updated_at", 5000),
    getRows<Record<string, unknown>>("product_master", "id,sku,display_for_packer,th_name,unit_price,emoji,stock_qty,stock_status", 5000),
  ]);
  const productBySku = new Map(products.map(product => [String(product.sku ?? "").trim(), product]));
  return rows.map((row, index) => ({
    ...
    canonicalLabel: text(productBySku.get(String(row.sku ?? "").trim())?.display_for_packer ?? productBySku.get(String(row.sku ?? "").trim())?.th_name ?? row.sku) ?? "",
    isActive: true,
...
export async function createProductAliasInSupabase(input: { alias: string; canonicalSku: string; canonicalLabel: string }) {
  ...
  const masterRows = await getRows<Record<string, unknown>>("product_master", "id,sku,display_for_packer,th_name", 5000);
  ...
  const body = { sku: input.canonicalSku.trim(), alias: cleanAlias, alias_text: cleanAlias, alias_norm: cleanAlias.toLowerCase(), source: "front_house", source_room: "front_house", updated_at: new Date().toISOString() };
...
export async function updateProductAliasInSupabase(id: number, input: { alias: string; canonicalSku: string; canonicalLabel: string; isActive?: boolean }) {
  ...
  if (!master) throw new Error(`ไม่พบ SKU ${input.canonicalSku} ใน product_master`);
  const body = { sku: input.canonicalSku.trim(), alias: cleanAlias, alias_text: cleanAlias, alias_norm: cleanAlias.toLowerCase(), source: "front_house", source_room: "front_house", updated_at: new Date().toISOString() };
  const response = await fetch(`${baseUrl}/rest/v1/product_map_master?map_id=eq.${encodeURIComponent(String(id))}`, { method: "PATCH", headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", Prefer: "return=minimal" }, body: JSON.stringify(body) });
