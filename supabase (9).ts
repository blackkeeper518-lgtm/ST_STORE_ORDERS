...
export async function listProductAliasesFromSupabase() {
  const select = "map_id,alias,alias_text,alias_norm,sku,mapping_status,match_confidence,source,source_room,updated_at,product_master_id,master_display_for_packer,master_th_name,master_unit_price,master_emoji,master_stock_qty,master_stock_status";
  let rows: Array<Record<string, unknown>>;
  try {
    rows = await getRows<Record<string, unknown>>("vw_product_map_with_master", select, 5000);
  } catch {
    // Keep the web room usable during PostgREST schema-cache/view migrations.
    // The table is authoritative for aliases; product_master remains the
    // source of truth for Display and stock on the next refresh.
    rows = await getRows<Record<string, unknown>>("product_map_master", "map_id,alias,alias_text,alias_norm,sku,mapping_status,match_confidence,source,source_room,updated_at,display_for_packer,th_name,unit_price,emoji", 5000);
  }
  return rows.map((row, index) => ({
    ...
    canonicalLabel: text(row.master_display_for_packer ?? row.display_for_packer ?? row.final_display_for_packer ?? row.canonical_label ?? row.master_th_name ?? row.th_name ?? row.sku) ?? "",
    isActive: true,
