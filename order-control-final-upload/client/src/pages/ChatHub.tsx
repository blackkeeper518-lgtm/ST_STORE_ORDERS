...
export default function ChatHub() {
  ...
  const threadsQuery = useQuery({ queryKey: ["canonical-chat-threads"], queryFn: readChatThreads, refetchInterval: 15_000 });
  ...
  const messagesQuery = useQuery({ queryKey: ["canonical-chat-messages", selectedPageId, selectedThreadId], queryFn: () => readChatMessages(selectedPageId, selectedThreadId), enabled: Boolean(selectedPageId && selectedThreadId), refetchInterval: 5_000 });
  useEffect(() => {
    const unsubscribe = subscribeToChatMessages(() => {
      void threadsQuery.refetch();
      if (selectedPageId && selectedThreadId) void messagesQuery.refetch();
    });
    return unsubscribe;
  }, [selectedPageId, selectedThreadId]);
  ...
  const openOrderSummary = () => {
    if (!selected) return;
    const linkedSource = selected.orders?.find((order: any) => order.source_text)?.source_text
      || selected.orders?.find((order: any) => order.source_payload?.source_text)?.source_payload?.source_text;
    const seed = linkedSource || selected.chatTimeline?.join("\n") || selected.preview || "";
    setSummary(null);
    setSummaryText(seed);
    setSummaryOpen(true);
  };
  ...
  const visibleTimeline = (selected?.chatTimeline ?? []).filter((line: string) => messageFilter === "all" || (messageFilter === "page" ? isPageTimelineLine(line) : isCustomerTimelineLine(line)));
  ...
  const composer = <div className="border-t border-orange-500/10 pt-2">
    <div className="mb-2 flex flex-wrap gap-1.5">
      {quickReplies.map(reply => <button key={reply} type="button" onClick={() => setReplyText(reply)} className="rounded-lg border border-orange-400/15 bg-orange-500/[0.06] px-2 py-1 text-[10px] text-orange-200/75 transition hover:bg-orange-500/20 hover:text-orange-100">{reply}</button>)}
      {["😊", "👍", "❤️", "🙏", "📦"].map(emoji => <button key={emoji} type="button" onClick={() => addEmoji(emoji)} className="rounded-lg border border-orange-400/15 bg-orange-500/[0.06] px-2 py-1 text-sm transition hover:bg-orange-500/20">{emoji}</button>)}
    </div>
    <div className="flex gap-2"><Input value={replyText} onChange={event => setReplyText(event.target.value)} placeholder={selectedCustomerId ? "พิมพ์ตอบลูกค้า…" : "ไม่พบ Customer PSID ของห้องนี้"} disabled={!selectedCustomerId} className="border-orange-500/15 bg-black/25 text-white placeholder:text-orange-100/25" onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendCurrentReply(); } }} /><Button aria-label="ส่งข้อความ" disabled={sendReply.isPending || !selectedCustomerId || (!replyText.trim() && !replyImageUrl.trim() && !stickerId.trim())} onClick={sendCurrentReply} className="shrink-0 rounded-xl bg-gradient-to-r from-orange-600 to-orange-600"><Send className="h-4 w-4" /></Button></div>
    <div className="mt-1 flex flex-wrap items-center gap-2"><input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden" onChange={event => { handleImageUpload(event.target.files?.[0]); event.currentTarget.value = ""; }} /><Button type="button" size="sm" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploadImage.isPending} className="h-7 border-orange-500/15 bg-black/20 text-[10px] text-orange-100/70"><ImagePlus className="mr-1 h-3.5 w-3.5" />{uploadImage.isPending ? "กำลังอัปโหลด…" : "แนบรูป"}</Button>{replyImageUrl ? <span className="max-w-40 truncate text-[10px] text-emerald-300">รูปพร้อมส่ง</span> : null}<Input value={stickerId} onChange={event => setStickerId(event.target.value)} placeholder="Sticker ID (ถ้ามี)" className="h-7 min-w-32 flex-1 border-orange-500/10 bg-black/20 text-xs text-white placeholder:text-orange-100/20" /><span className="whitespace-nowrap text-[10px] text-orange-100/25">Meta</span><Button type="button" size="sm" variant="outline" onClick={() => simulate("text")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-cyan-400/20 bg-cyan-400/[0.04] text-[10px] text-cyan-200">จำลองข้อความ</Button><Button type="button" size="sm" variant="outline" onClick={() => simulate("image")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-cyan-400/20 bg-cyan-400/[0.04] text-[10px] text-cyan-200">จำลองรูป</Button></div>
    ...
    {deliveryStatus !== "idle" ? <div className={`mt-2 flex items-center justify-between rounded-lg border px-2.5 py-1.5 text-[11px] ${deliveryStatus === "sent" ? "border-emerald-400/20 bg-emerald-400/[0.06] text-emerald-300" : deliveryStatus === "failed" ? "border-red-400/20 bg-red-400/[0.06] text-red-300" : "border-orange-400/20 bg-orange-400/[0.06] text-orange-200"}`}><span>{deliveryStatus === "sending" ? "กำลังส่ง…" : deliveryStatus === "sent" ? "ส่งแล้ว" : "ส่งไม่สำเร็จ"}</span>{deliveryStatus === "failed" && !isThreadControlConflict(sendReply.error?.message ?? "") ? <button type="button" onClick={sendCurrentReply} className="font-semibold underline">ลองส่งใหม่</button> : null}</div> : null}
  ...
  const conversation = selected ? <div className="flex min-h-0 flex-col overflow-hidden">
    <div className="flex items-center justify-between gap-3 border-b border-orange-500/10 px-5 py-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-orange-500/25 to-orange-500/25 text-orange-200"><CircleUserRound className="h-5 w-5" /></div><div className="min-w-0"><h2 className="truncate text-base font-semibold text-white">{selected.customerName || "ลูกค้าใหม่"}</h2><div className="mt-1 flex items-center gap-2 text-[11px] text-orange-100/40"><span className="truncate">{selected.pageName}</span><span>·</span><span>{timeLabel(selected.latestAt)}</span></div></div></div><div className="flex items-center gap-2"><Badge className="hidden border border-emerald-400/25 bg-emerald-400/10 text-emerald-300 sm:inline-flex">● ONLINE</Badge><Button size="sm" variant="outline" onClick={confirmSelectedOrder} disabled={selectedConfirmed || confirmOrder.isPending} className="rounded-xl border-cyan-400/25 bg-cyan-400/10 text-xs text-cyan-200 hover:bg-cyan-400/20">{selectedConfirmed ? <><CheckCircle2 className="mr-1 h-3.5 w-3.5" />ยืนยันแล้ว</> : confirmOrder.isPending ? "กำลังบันทึก…" : "ยืนยันเป็นออเดอร์"}</Button><Button size="sm" variant="outline" onClick={() => setOrderDetailOpen(true)} className="rounded-xl border-amber-400/25 bg-amber-400/10 text-xs text-amber-200 hover:bg-amber-400/20">ออเดอร์ ({linkedOrdersQuery.data?.length ?? 0})</Button><Button size="sm" onClick={openOrderSummary} className="rounded-xl bg-orange-600/80 text-xs hover:bg-orange-500">สรุปออเดอร์</Button>{isMobile ? <button onClick={() => setMobileOpen(false)} className="rounded-xl p-2 text-orange-100/55 hover:bg-orange-500/10 hover:text-white" aria-label="ปิดห้องแชท"><X className="h-5 w-5" /></button> : null}</div></div>
    <div ref={chatTimelineRef} onScroll={handleTimelineScroll} className="relative min-h-0 flex-1 flex flex-col justify-end gap-3 overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_70%_20%,rgba(168,85,247,0.09),transparent_32%),#0b0910] p-5 [-webkit-overflow-scrolling:touch]">
      ...
      {visibleStoredMessages.length === 0 ? <>{visibleTimeline.length ? visibleTimeline.map((line: string, index: number) => <div key={`${line}-${index}`} className={`max-w-[82%] min-w-0 shrink-0 break-words [overflow-wrap:anywhere] rounded-2xl px-4 py-3 text-sm leading-6 shadow-lg ${isPageTimelineLine(line) ? "ml-auto rounded-br-md bg-gradient-to-br from-orange-600/80 to-orange-600/80 text-white" : "rounded-bl-md border border-white/10 bg-white/[0.06] text-orange-50/85"}`}><p className="whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{line}</p></div>) : <div className="rounded-2xl border border-orange-500/10 bg-black/20 p-4 text-sm leading-6 text-orange-100/60"><p>{selected.preview}</p></div>}<p className="text-[11px] text-orange-100/30">แชทจากตารางกลาง · n8n API รอบถัดไปจะอัปเดตเข้าห้องนี้อัตโนมัติ</p></> : null}
      {visibleStoredMessages.map(message => { const isRight = ("side" in message && message.side === "right") || message.direction === "outbound" || message.senderType === "page"; const senderName = ("senderName" in message ? message.senderName : null) ?? (isRight ? "เพจ / แอดมิน" : "ลูกค้า"); return <div key={message.id} className={`flex w-full ${isRight ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] min-w-0 shrink-0 break-words [overflow-wrap:anywhere] rounded-2xl px-4 py-3 text-sm leading-6 shadow-lg ${isRight ? "rounded-br-md bg-gradient-to-br from-orange-600/80 to-orange-600/80 text-white" : "rounded-bl-md border border-white/10 bg-white/[0.06] text-orange-50/85"}`}><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide opacity-60">{senderName}</p><p className="whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{message.text || "ส่งรูปภาพ"}</p>{imageUrls(message.attachmentsJson).map(url => <button key={url} type="button" onClick={() => setLightboxUrl(url)} className="mt-2 block cursor-zoom-in text-left"><img src={url} alt="ไฟล์แนบจากแชท คลิกเพื่อขยาย" className="max-h-56 rounded-xl object-cover transition hover:brightness-110" /></button>)}<p className="mt-1 text-[10px] opacity-50">{isRight ? "เพจ / แอดมิน" : "ลูกค้า"} · {timeLabel(String(message.occurredAt))}</p></div></div>; })}
    </div>
    <div className="border-t border-orange-500/10 bg-[#100d15] p-4"><div className="mb-3 flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[10px] uppercase tracking-wider text-orange-100/35">ดูข้อความ</span>{([ ["all", "ทั้งหมด"], ["customer", "ลูกค้า"], ["page", "เพจ"] ] as const).map(([value, label]) => <button key={value} onClick={() => setMessageFilter(value)} className={`rounded-lg px-2.5 py-1 text-[10px] transition ${messageFilter === value ? "bg-orange-500/20 text-orange-200 ring-1 ring-orange-400/30" : "text-orange-100/45 hover:bg-orange-500/10 hover:text-orange-100"}`}>{label}</button>)}</div>{summary ? <div className="mb-3 rounded-2xl border border-orange-400/20 bg-orange-500/[0.06] p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-orange-200">พรีวิวใบสรุป · {summary.orderNumber}</p><button onClick={() => setSummary(null)} className="text-[10px] text-orange-100/40 hover:text-white">ล้าง</button></div><p className="mt-1 text-[11px] text-orange-100/45">ตรวจข้อความด้านล่างให้ถูกต้องก่อนกดส่งให้ลูกค้า</p><Button variant="outline" size="sm" onClick={copySummary} className="mt-2 h-7 border-orange-400/20 bg-transparent text-[10px] text-orange-200">{copied ? <Check className="mr-1 h-3 w-3" /> : <Clipboard className="mr-1 h-3 w-3" />}{copied ? "คัดลอกแล้ว" : "คัดลอกข้อความสรุป"}</Button></div> : null}{composer}</div>
  </div> : <div className="flex min-h-[560px] items-center justify-center text-sm text-orange-100/35">เลือกห้องแชทเพื่อเริ่มดูข้อความ</div>;
