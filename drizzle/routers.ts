...
export const appRouter = router({
  ...
  orders: router({
    threads: publicProcedure.query(async () => {
      // Build the thread list only from the two existing chat rooms and st_orders.
      const [customers, pages, orders] = await Promise.all([
        ...
        supabaseGet<Row[]>("st_orders?select=*&order=created_at.desc&limit=3000"),
      ]);
    ...
    live: publicProcedure.input(z.object({ search: z.string().optional(), limit: z.number().int().min(1).max(500).default(300) }).optional()).query(async ({ input }) => {
      ...
      const rows = await supabaseGet<Row[]>(`st_orders?${params.join("&")}`);
    ...
    dailyOrderHistory: publicProcedure.input(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), search: z.string().optional() })).query(async ({ input }) => {
      // Read the existing st_orders table directly. The date is compared in
      // Bangkok time so the UI's date picker matches the shop's business day.
      const rows = await supabaseGet<Row[]>("st_orders?select=*&order=created_at.desc&limit=3000");
    ...
    forThread: publicProcedure.input(threadInput).query(async ({ input }) => {
      ...
      let rows: Row[] = [];
      try {
        rows = await supabaseGet<Row[]>(`st_orders?page_id=eq.${page}&thread_id=eq.${thread}&select=*&order=created_at.desc`);
      } catch {
        // Some older st_orders versions used conversation_key instead of thread_id.
        rows = await supabaseGet<Row[]>(`st_orders?page_id=eq.${page}&conversation_key=eq.${thread}&select=*&order=created_at.desc`);
      }
