...
-- The RPC intentionally returns st_orders rows only. Product details remain in bb_order_items_fix.

create index if not exists st_orders_page_thread_created_idx
  on public.st_orders (page_id, thread_id, created_at desc);

create index if not exists st_orders_page_id_idx
  on public.st_orders (page_id);

create index if not exists st_orders_thread_id_idx
  on public.st_orders (thread_id);
...
returns setof public.st_orders
language sql
stable
security invoker
