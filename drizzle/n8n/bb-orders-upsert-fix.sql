...
-- n8n URL uses: /rest/v1/st_orders?on_conflict=upsert_key
...
create unique index if not exists st_orders_upsert_key_unique_idx
  on public.st_orders (upsert_key);

comment on index public.st_orders_upsert_key_unique_idx is
...
where schemaname = 'public'
  and tablename = 'st_orders'
  and indexname = 'st_orders_upsert_key_unique_idx';

-- n8n HTTP Request settings:
-- Method: POST (or PATCH/upsert operation)
-- URL: /rest/v1/st_orders?on_conflict=upsert_key
-- Header: Prefer = resolution=merge-duplicates,return=minimal
-- Body: output from ST_ORDERS_CANONICAL_PAYLOAD
