...
-- ใช้กับ public.st_orders หลังตรวจว่าคอลัมน์ telegram_body และ display_for_packer มีอยู่แล้ว
-- ไม่ลบ/ไม่เขียนทับ items_json ที่มีข้อมูลอยู่แล้ว

create or replace function public.st_orders_fill_items_json_fallback()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  -- fallback 2: legacy single product columns already present in st_orders
  if jsonb_array_length(candidate) = 0 and (
    ...
    candidate := jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      ...
      'quantity', 1,
      'unit_price', null,
      'fallback_source', 'st_orders_legacy_columns'
    )));
  end if;
...
drop trigger if exists st_orders_fill_items_json_fallback on public.st_orders;
create trigger st_orders_fill_items_json_fallback
before insert or update of items_json, telegram_body, sku, th_name, display_label, display_for_packer
on public.st_orders
for each row execute function public.st_orders_fill_items_json_fallback();

-- ตรวจสอบรายการที่ยังว่างหลังติดตั้ง trigger
comment on function public.st_orders_fill_items_json_fallback() is
  'Fills empty st_orders.items_json from telegram_body payload or legacy product columns without overwriting non-empty items';
