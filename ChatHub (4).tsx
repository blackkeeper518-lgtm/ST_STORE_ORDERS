...
import { readChatMessages, readChatThreads } from "@/lib/canonical";
...
export default function ChatHub() {
  ...
  const threadsQuery = useQuery({ queryKey: ["canonical-chat-threads"], queryFn: readChatThreads, refetchInterval: 60_000 });
  ...
  const messagesQuery = useQuery({ queryKey: ["canonical-chat-messages", selectedPageId, selectedThreadId], queryFn: () => readChatMessages(selectedPageId, selectedThreadId), enabled: Boolean(selectedPageId && selectedThreadId), refetchInterval: 10_000 });
